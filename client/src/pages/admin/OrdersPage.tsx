...
const OrdersPage = () => {
  ...
  const handleSearchChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // تعيين تأخير جديد للبحث (debounce حقيقي)
    const timeout = setTimeout(() => {
      setPage(1); // إعادة تعيين الصفحة إلى 1 عند البحث
      fetchOrders();
    }, 800); // زيادة الوقت إلى 800ms
  ...
  const fetchOrders = async () => {
    try {
      ...
      setError(err.response?.data?.message || err.message || t('orders.loadError'));
  ...
  useEffect(() => {
    fetchOrders();
    // تنظيف التأخير عند إزالة المكون
    return () => {
      if (searchTimeout) {
        clearTimeout(searchTimeout);
      }
    };
  }, [page, statusFilter, paymentFilter]); // إزالة search من التبعيات لأننا نتعامل معه في handleSearchChange
  ...
  const handleStatusChange = async (orderId: string, newStatus: OrderStatus) => {
    try {
      ...
      // تحديث الطلب في القائمة مباشرة
      const updatedOrders = orders.map(order => 
        order._id === orderId ? { ...order, status: newStatus, isPaid: newStatus === 'delivered' } : order
      );
      dispatch({ type: 'orders/setOrders', payload: updatedOrders });
      
      // تحديث orders في الكومبوننت
      setOrders(updatedOrders);
      
      // تحديث إحصائيات لوحة التحكم
      const order = orders.find(order => order._id === orderId);
      if (order) {
        dispatch(updateDashboardAfterOrderChange({
    ...
    <Box className="p-4">
      ...
      <Paper className="p-4 mb-4">
        <Grid container spacing={2} alignItems="center">
          ...
          <Grid item xs={12} md={3}>
            <FormControl fullWidth variant="outlined">
              <InputLabel>{t('orders.filterByPayment') || 'Payment Status'}</InputLabel>
              <Select
                value={paymentFilter}
              ...
              >
                ...
                <MenuItem value="unpaid">{t('orders.unpaid') || 'Unpaid'}</MenuItem>
              </Select>
            </FormControl>
          </Grid>
          <Grid item xs={12} md={3}>
          ...
          <TableBody>
            {orders.map((order, idx) => (
              <TableRow key={order._id || order.id || idx}>
                <TableCell>
                  {order._id || order.id ? (
                    <Link to={`/admin/orders/${order._id || order.id}`} className="text-primary-600 hover:underline">
                ...
                <TableCell>
                  <Box>
                    <Typography variant="body2" className="font-medium">
                      {order.guestInfo?.name || (typeof order.user === 'object' && order.user?.name ? order.user.name : '-')}
                    </Typography>
                  </Box>
                </TableCell>
                <TableCell>
                  <Typography variant="body2">
                    {order.guestInfo?.phone || (typeof order.user === 'object' && order.user?.phone ? order.user.phone : '-')}
                  </Typography>
                </TableCell>
                ...
                <TableCell>
                  {order.isPaid ? (
                    <Chip label={t('orders.paid') || 'Paid'} color="success" size="small" />
                  ) : (
                    <Chip label={t('orders.unpaid') || 'Unpaid'} color="error" size="small" />
                  )}
                </TableCell>
                <TableCell>
                  <Chip 
                ...
                <TableCell>
                    ...
                    {order.orderItems && order.orderItems.length > 0 ? (
                      order.orderItems.slice(0, 2).map((item: any, idx: number) => (
                        <img
                          ...
                          src={item.image || (item.product && item.product.image) || '/images/product-placeholder.svg'}
                          alt={item.name}
                          style={{ width: 32, height: 32, objectFit: 'cover', marginRight: 4, borderRadius: 4 }}
                        />
                      ))
                    ) : (
                      <span>-</span>
                  ...
                  <Box>
                    {order.orderItems && order.orderItems.length > 0
                      ? order.orderItems.slice(0, 2).map((item: any, idx: number) => (
                          <Typography key={item._id || item.productId || idx} variant="caption" display="block">
                            {item.name}
                          </Typography>
                        ))
                      : <span>-</span>
                ...
                <TableCell align="right">
                  <IconButton
                    ...
                    to={order._id || order.id ? `/admin/orders/${order._id || order.id}` : undefined}
                    color="primary"
                    title={t('common.view')}
                  ...
                  <IconButton
                    onClick={() => handlePrintInvoice(order._id || order.id || '')}
                    color="secondary"
                  ...
                  <IconButton
                    onClick={() => handleDeleteClick(order._id || order.id || '')}
                    color="error"
                  ...
                  <IconButton
                    onClick={(e) => handleOpenMenu(e, order._id || order.id || '')}
      ...
      >
        <MenuItem onClick={() => selectedOrderId && handleStatusChange(selectedOrderId, 'processing')}>
          {t('orders.status.processing')}
        </MenuItem>
        <MenuItem onClick={() => selectedOrderId && handleStatusChange(selectedOrderId, 'shipped')}>
          {t('orders.status.shipped')}
        </MenuItem>
        <MenuItem onClick={() => selectedOrderId && handleStatusChange(selectedOrderId, 'delivered')}>
          {t('orders.status.delivered')}
        </MenuItem>
        <MenuItem onClick={() => selectedOrderId && handleStatusChange(selectedOrderId, 'cancelled')}>
